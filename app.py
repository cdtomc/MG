...
# 모바일 화면 극대화를 위한 울트라 초밀착 여백 및 피드백 기반 타겟 컬러 지정 CSS 스타일
st.markdown("""
    <style>
    /* 1. 전체 화면 및 요소 간격 조밀화 */
    .block-container {
    ...
    /* 3. 🎨 목표 설정 원터치 버튼군 지정 컬러 오버라이드 고정 (피드백 반영) */
    ...
    /* 4. 핵심 4대 지표 입력창 소프트 블루 인디고 하이라이트 주입 */
...
if 'cat_rate' not in st.session_state: st.session_state.cat_rate = platform_db[st.session_state.selected_platform]["cat"]
if 'link_rate' not in st.session_state: st.session_state.link_rate = platform_db[st.session_state.selected_platform]["link"]
if 'ship_rate' not in st.session_state: st.session_state.ship_rate = platform_db[st.session_state.selected_platform]["ship"]
...
# [피드백 2 반영] 음수 입력 방지 및 순수 숫자만 추출하도록 튼튼하게 개선한 파싱 함수
def parse_money(value):
    cleaned = re.sub(r"[^0-9]", "", str(value))
    try: return max(0, int(cleaned))
    except: return 0
...
with top_container:
    ...
    with col_head2: st.selectbox("쇼핑몰 선택", list(platform_db.keys()), key="selected_platform", label_visibility="collapsed", on_change=sync_platform_fees)
...
# 4. 수수료 세부 설정 [피드백 3 반영: 최솟값 및 최댓값 한계 제한 구현]
st.markdown("---")
st.subheader("🛒 수수료 세부 설정")
col_fee1, col_fee2, col_fee3 = st.columns(3)
with col_fee1: cat_rate = st.number_input("카테고리 (%)", min_value=0.0, max_value=99.0, step=0.1, key="cat_rate")
with col_fee2: link_rate = st.number_input("연동 (%)", min_value=0.0, max_value=99.0, step=0.1, key="link_rate")
with col_fee3: ship_rate = st.number_input("배송비 (%)", min_value=0.0, max_value=99.0, step=0.1, key="ship_rate")

vat_rate = st.number_input("부가세율 (%)", min_value=0, max_value=100, value=10, step=1)
...
# [피드백 7번 유지] 부동소수점 오차 방지를 위한 round() 반올림 보정 엔진
def calc_forward_metrics(sell_price):
    total_sales = sell_price + customer_shipping
    platform_fee = (sell_price * (st.session_state.cat_rate + st.session_state.link_rate) / 100) + (customer_shipping * (st.session_state.ship_rate / 100))
...
# 순수익 타겟 기반 판매가 무조건 올림(math.ceil) 역산식
def price_from_profit(target_profit):
    ...
    if fee_denom > 0:
        price = math.ceil((target_settlement - customer_shipping * (1 - st.session_state.ship_rate / 100)) / fee_denom)
        return max(0, price)
    return 0
...
with top_container:
    ...
    # [피드백 1 반영] 중복 선언된 profits_layout 제거 및 깔끔하게 한 줄 단축 정렬
...
with st.expander("🔍 상세 정산 데이터 확인"):
    ...
    # [피드백 5 반영] 상세 정산서 내 실제 ROI 데이터 표기 추가
    ...
    # [피드백 4 반영] 과장 섞이지 않은 담백한 부동소수점 오차 보정 가이드 및 부가세 가정 캡션 노출
